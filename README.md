...
明瞭な標準Native発話は、providerのendpoint通知と端末VADが一致した場合に280 ms、通知がない場合も400 msの局所無音でturnを確定します。静かな発話、長い独話、回答支援中の継続発話にはこの短縮を適用せず、それぞれの長い待機窓を維持します。最初の有意味PCMを端末出力時刻へ配置できた時だけ、発話末尾から可聴開始までの推定値を「返答開始 約N.N秒」と現在sessionの画面へ表示します。無音frame、受領表示、過去turnの値は表示根拠にしません。
