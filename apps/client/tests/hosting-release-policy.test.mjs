...
test("Hosting release binds one clean origin/main commit to immutable artifacts", async () => {
  ...
  assert.match(
    deploy,
    /\[bool\]\s*\$result\.observationAddingValidated\s+-ne\s+\$true/u,
  );
  assert.match(
    deploy,
  ...
  assert.match(
    deploy,
    /\[bool\]\s*\$result\.acousticCoverageWireValidated\s+-ne\s+\$true/u,
  );
  assert.match(
    deploy,
  ...
  assert.match(
    deploy,
    /\[bool\]\s*\$result\.quietSpectralCompensationValidated\s+-ne\s+\$true/u,
  );
  assert.match(
    deploy,
    /\[bool\]\s*\$result\.quietSubbandEvidenceValidated\s+-ne\s+\$true/u,
  );
  assert.match(
    deploy,
    /\[bool\]\s*\$result\.guestAFirstSprintSloValidated\s+-ne\s+\$true/u,
  );
  assert.match(
    deploy,
