...
class KotaePcmCaptureProcessor extends AudioWorkletProcessor {
  ...
  clearFallbackRing() {
    ...
    for (const ring of [this.fallbackRing, this.baselineFallbackRing]) {
      ...
      try {
        ...
        // The processor remains fail-closed if either Wasm owner faulted.
      }
    }
  }
  ...
  dropFallbackPair() {
    ...
    for (const ring of [this.fallbackRing, this.baselineFallbackRing]) {
      try {
        ...
        // Both bounded owners were cleared before release.
      }
    }
    this.fallbackRing = undefined;
    this.baselineFallbackRing = undefined;
  ...
  enqueueConfirmed(entry, baselinePcm) {
    if (
  ...
  normalizeConfirmedQuietSpeech(entry) {
    ...
    try {
      const result = this.confirmedQueue.compensateQuietFrame(
        this.generation,
        new Uint8Array(entry.pcm),
      );
      const phaseIntegrity = this.confirmedQueue.quietPhaseIntegrity(
        this.generation,
      );
      return (
        result !== QUIET_COMPENSATION_INVALID &&
        Number.isSafeInteger(phaseIntegrity) &&
        phaseIntegrity >= 0 &&
        phaseIntegrity <= 5 &&
        (result !== 2 || phaseIntegrity === 1)
      );
    } catch {
      return false;
    }
  ...
  baselineCopy(entry) {
    if (!this.fallbackRing || !this.baselineFallbackRing) return undefined;
    if (
      !(entry.pcm instanceof ArrayBuffer) ||
      entry.pcm.byteLength !== FRAME_BYTES
    ) {
      return undefined;
    }
    return entry.pcm.slice(0);
  }
  ...
  completeFrame(entry) {
    ...
    if (this.state === "confirmed") {
      ...
      const baselinePcm = this.baselineCopy(entry);
      if (this.fallbackRing && !(baselinePcm instanceof ArrayBuffer)) {
        zeroizeBuffer(entry.pcm);
        this.failClosed();
        return;
      }
      ...
      this.enqueueConfirmed(entry, baselinePcm);
      return;
    }
    zeroizeBuffer(entry.pcm);
  }
  ...
  confirm(control) {
    if (
      ...
      typeof control.aecVerified !== "boolean" ||
    ...
    this.quietGainEnabled = control.quietConfirmed && control.aecVerified;
    if (this.quietGainEnabled) {
      try {
        ...
        this.baselineFallbackRing = createPcmRing(
          this.pcmRingModule,
          this.generation,
          MAXIMUM_HTTP_FALLBACK_FRAMES,
          false,
  ...
  takeFallback(control) {
    ...
    const baselineRemaining = this.countRing(
      this.baselineFallbackRing,
      MAXIMUM_HTTP_FALLBACK_FRAMES,
    );
    ...
    while (remaining > 0) {
      ...
      try {
        for (let index = 0; index < frameCount; index += 1) {
          ...
          const baselineDestination = new Uint8Array(
            baselinePcm,
            index * FRAME_BYTES,
            FRAME_BYTES,
          );
          ...
          const baselineContextFrame = this.baselineFallbackRing.shiftInto(
            this.generation,
            baselineDestination,
          );
          if (
