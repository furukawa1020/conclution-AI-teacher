...
本人の最終意図が「代わりに答えないで」「回答を作らないで」「この回答を読み上げないで」という直接の拒否なら、回答支援への同意にはしない。Native Audioは同じ有限所有権文法で拒否を確定し、そのturnでproviderが準備したPCMと出力字幕を破棄して、final captionを二回目のSTTなしで監査済みstaged controllerへ一度だけ渡す。controllerはモデルを呼ばず固定確認だけを返し、新しいRespondent checkpointやproofを発行しない。途中captionが代理回答依頼だった場合もprovider sessionを汚染済みとして保持し、finalで撤回・引用・伝聞へ変わってもprovider出力を通常会話として公開しない。拒否は既存の質問拘束済みscopeを自動解除せず、その認証済み制御stateをそのまま保つ。
...
complete / completeでverifiedになったturnは回答所有権yieldとし、KOTAEは相づち・講評・成功音声を合成しません。HTTPとNative caption handoffのどちらも空のspoken replyによってTTSを呼ばず、暗号化stateと固定proof enumだけをfinalへ載せます。さらにvoiceflow境界は、terminal proofまたはprivate candidateと非空spoken replyが同居する矛盾をPCM生成前に拒否します。画面はこの組を「AIの返答が失敗した無音」と区別し、現在turnだけ回答所有権レシートとして表示します。expanding / expandは利用者が事前に明示した一問を音声で返すためyieldではありません。
