...
短い明瞭な標準Native発話のendpointは、provider通知と端末VADの二重一致なら280 ms、端末単独のfallbackなら400 msです。静かな発話、長い独話、active Coachは従来の保護窓を維持します。ブラウザは最初の有意味PCMをoutput timestampへ配置した時だけ、speech-endから推定可聴開始までのcontent-free値をcurrent-turn eventとしてWasmへ渡します。Wasmは完全一致schema、version、0〜120秒の有限範囲を検証し、現在sessionだけに「返答開始 約N.N秒」を表示します。この値は端末出力時刻に基づく推定であり、保存、能力評価、意味理解の証明には使いません。
