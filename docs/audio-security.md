...
Cloud Run kotae-api（asia-northeast1）
  ...
  │                                   └─ 初回回答支援 ──→ Cloud Run内のモデルなし署名checkpoint ──→ ブラウザ
...
| Native Audio caption / 音声 | Vertex AI `us-central1`、Cloud Run、ブラウザ。初回回答支援の入力captionを別の文字列modelへ渡さない | なし | 発話本文を状態tokenへ入れない |
...
| 会話状態 | 標準時だけブラウザメモリ、次ターンのCloud Run | サーバーDBには保存しない | 通常Native turnは発話本文を含まないlease。初回回答支援は用途分離した非可逆scope tagと有限coach制御metadataだけ。段階的な標準turnはフィルタ済み意味グラフと有限coach制御metadata。15分TTL。厳格時は空 |
...
- 確認前PCMはAudioWorklet内だけに保持し、通常発話は最大75 frame、割り込みは最大45 frameに固定する。再生中の短いぼやきや相づちは再生を変えず160 msで端末内の仮候補に留め、400 msの持続音声と0.65以上のvoice densityを満たした時だけ応答を中断する。VAD確認後はAudioContextのsample-clock cutoff、session generation、連続sequenceを検証し、credit制御でMessagePortの未処理数も固定する。turn確定は全PCMの`sealed`確認後だけ許可する
