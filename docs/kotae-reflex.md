...
`respondent`経路の目的は、KOTAEが整えた答えを代読することではありません。通常会話では無効とし、本人が「答え方を一問だけ手伝って」などと明示的に頼んだ時だけ、一つの固定された短い質問を出します。本人の発話内にAがあれば、理由や前置きの後ろにあっても、Aが話の中に見つかったことだけを固定の短い言葉で返して支援を閉じます。Aの引用、AIによる整文、自動的な言い直し要求はせず、その後は通常会話へ戻ります。Aを安全に特定できなければ答えを捏造せず、一度の短い問いで難しい時は支援を解きます。
...
段階フェードは、有限コーチの固定質問を選択式から自由質問へ変える機能ではありません。本人が明示的に頼んだ練習で「回答を先に置けた」と独立検証できた後、前述の短期・非意味メタデータにより、通常会話でAIが任意に添える質問の量と型だけを`guided`、`light`、`natural`へ少しずつ調整します。`guided`は答えやすい短い一問、`light`は短い自由回答の一問、`natural`は通常の会話です。「今日は話すだけ」では通常会話だけにし、直後のcooldown中は任意質問を足しません。段階、score、連続成功は本人へ表示しません。
