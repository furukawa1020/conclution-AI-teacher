...
Cloud Run / Go（asia-northeast1）
  ...
  │    └─ 初回の明示回答支援: Native音声を即時解放 + final input captionを並行してRespondent Coachへ
...
- 端末VADで一発話を区切り、録音開始から最大3分30秒まで認証付きWebSocketでCloud Runへ20 ms PCMを増分送信する。標準のPDFなしlive turnはまず`us-central1`のVertex AI Native Audioへ中継する。本人自身が回答支援を明示したことをfinal入力captionから検証した場合は、同じcaptureを再送せず、Native音声の解放とcaptionからの署名済みcoach state生成を並行する。厳格モード・PDF・回答支援の保留中・live接続fallbackは段階経路を使う。発話が確定しない無音候補は最大30秒で終了するため、その上限直前から話し始めても約3分は残るが、3分30秒の実発話を保証するものではない。Cloud Run側は最大4分、12,000 frame・7,680,000 byteまで受ける。Goのlive接続deadlineは6分、Cloud Runのrequest timeoutは420秒で、4分のcapture後にもcommitとモデル処理の時間を残す。
...
- 初回回答支援はNative入力処理と音声応答をそのまま使い、captionからの文字列推論と署名state生成を音声開始と並行する。commitから最初の音声frameまで1,000 ms以内を通常Nativeと同じ運用SLOとして`route=native-respondent-coach`で集計し、再送・STT・TTSの直列待ちを入れない。
...
- 本人の明示回答支援をfinal入力captionで検証した初回turnは、Native音声を即時解放し、同じcaptionをstructured reasoner / LAC / Respondent Coachへ並行投入して署名stateを作る。同じcaptureはSTTへ再送しない。保留中の次turnは東京リージョンSTT → `global` Vertex AI structured reasoner / LAC / Respondent Coach → 東京リージョンTTSを維持し、`complete` / `release`後の次turnからNativeへ戻る
