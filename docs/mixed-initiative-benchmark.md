...
## 人手holdout annotation

人手評価の素材本文はローカルに置き、Gitへ追加しない。
公開可能なannotation artifactは素材全体のSHA-256、盲検順序seed、fixture ID、有限action、匿名のrater slotだけを持つ。
氏名、メールアドレス、発話本文、自由記述はschemaで受け取らない。

各fixtureは2名以上20名以下で評価する。
明示的な「曖昧」、同数首位、決定票2未満、または首位一致率66.67%未満は曖昧caseとして成功・失敗の分母から外す。
ただし曖昧件数は削除せず報告する。
評価者間一致は名義尺度のKrippendorff's alphaを決定的に一万分率へ丸めて報告する。
