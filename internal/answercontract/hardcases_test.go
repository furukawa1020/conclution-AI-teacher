...
func TestJapaneseAnswerToAnswerInvariantHardCases(t *testing.T) {
	cases := []hardCase{
		...
		{
			name: "pdf p4 definition direct", question: "論文のこの言葉ってどういうこと？",
			answer:   "この論文での「意味エントロピー」は、意味の異なる回答群の不確実性です。",
			operator: OperatorDefinition, required: slots(SlotDefinition, SlotScope),
			filled: slots(SlotDefinition, SlotScope), hypotheses: []float64{0.99},
			position: PositionFirst, calibration: CalibrationCommitted,
			issue: IssueNone,
			first: "この論文での「意味エントロピー」は、意味の異なる回答群の不確実性です",
			want:  OutcomeKeep,
		},
		{
			name: "pdf p4 definition answer first", question: "論文のこの言葉ってどういうこと？",
			answer:   "著者は第三節でこの語を使っています。この論文での「意味エントロピー」は、意味の異なる回答群の不確実性です。",
			operator: OperatorDefinition, required: slots(SlotDefinition, SlotScope),
			filled: slots(SlotDefinition, SlotScope), hypotheses: []float64{0.99},
			position: PositionLater, calibration: CalibrationCommitted,
			issue:   IssueBackgroundFirst,
			first:   "この論文での「意味エントロピー」は、意味の異なる回答群の不確実性です",
			minimal: "この論文での「意味エントロピー」は、意味の異なる回答群の不確実性です",
			reconstructed: "この論文での「意味エントロピー」は、意味の異なる回答群の不確実性です。" +
				"著者は第三節でこの語を使っています。",
			meaning: 0.98, gain: 0.32, want: OutcomeRestructure,
		},
		{
			name: "pdf p5 project purpose direct", question: "結局このプロジェクトで何をやりやりたいの？",
			answer:   "このプロジェクトの目的は、答えの核を自然に先へ出せるよう支えることです。",
			operator: OperatorPurpose, required: slots(SlotPurpose), filled: slots(SlotPurpose),
			hypotheses: []float64{0.99}, position: PositionFirst,
			calibration: CalibrationCommitted, issue: IssueNone,
			first: "このプロジェクトの目的は、答えの核を自然に先へ出せるよう支えることです",
			want:  OutcomeKeep,
		},
		{
			name: "pdf p5 project purpose answer first", question: "結局このプロジェクトで何をやりやりたいの？",
			answer:   "音声AIと構造化推論を組み合わせています。このプロジェクトの目的は、答えの核を自然に先へ出せるよう支えることです。",
			operator: OperatorPurpose, required: slots(SlotPurpose), filled: slots(SlotPurpose),
			hypotheses: []float64{0.99}, position: PositionLater,
			calibration: CalibrationCommitted, issue: IssueBackgroundFirst,
			first:   "このプロジェクトの目的は、答えの核を自然に先へ出せるよう支えることです",
			minimal: "このプロジェクトの目的は、答えの核を自然に先へ出せるよう支えることです",
			reconstructed: "このプロジェクトの目的は、答えの核を自然に先へ出せるよう支えることです。" +
				"音声AIと構造化推論を組み合わせています。",
			meaning: 0.98, gain: 0.32, want: OutcomeRestructure,
		},
		{
			name:     "pdf p5 purpose repair preserves condition",
			question: "結局このプロジェクトで何をやりやりたいの？",
			answer:   "検証で有効性を確かめられたなら、会話の負荷を下げることが目的です。",
			operator: OperatorPurpose, required: slots(SlotPurpose, SlotCondition),
			filled: slots(SlotPurpose, SlotCondition), hypotheses: []float64{0.99},
			position: PositionFirst, calibration: CalibrationConditional,
			issue:         IssueConditionSeparated,
			first:         "会話の負荷を下げることが目的です",
			minimal:       "会話の負荷を下げることが目的です",
			reconstructed: "会話の負荷を下げることが目的です",
			meaning:       0.99, gain: 0.32, want: OutcomeReject,
		},
		{
