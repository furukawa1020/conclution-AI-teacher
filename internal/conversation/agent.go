...
const (
	...
	phaticLocalSpokenReply                 = "こんにちは。今日は私から一つ。音楽は、同じ曲でも朝と夜で印象が変わることがあります。思いつけば一言、聞くだけでも大丈夫です。"
	listenOnlyLocalSpokenReply             = "わかりました。今日は聞くだけで大丈夫です。では私から一つ。音は、同じ曲でも朝と夜で印象が変わることがあります。"
...
func proactiveTopicReply(turn int) string {
	switch turn % 4 {
	case 1:
		return "今日は小さな選択の話にします。予定のない時間は、何もしないのも一つの過ごし方です。今なら、静かな音と明るい音では、どちらが楽ですか？ パスでも大丈夫です。"
	case 2:
...
- 会話の最初の数turnはAI側が会話の負荷を多めに持つ。個人史を聞かず、質問なしでも意味が通る短い小話を先に出す。確認が必要でも二択一問までにし、パスや聞くだけを残す。
...
- 通常の雑談は原則一文から三文、四十文字から百文字程度で、内容への応答、関連する小話、必要なら質問一問の順にする。本人が聞くだけを望む時は質問を付けず、開示量や難易度を自動で上げない。
...
- 短い発話、ぼやき、一語の話題、話題提供の依頼には、原則として二文から四文で返す。最小限の受け取り、AI側からの自足した内容一つ、必要なら答えやすい一問までの順にし、質問だけで終えない。
