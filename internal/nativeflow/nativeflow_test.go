...
func (f fakeRefresher) RefreshStateToken(string, string) (string, error) {
...
func TestNativeFlowRejectsInvalidStateBeforeOpeningProvider(t *testing.T) {
	...
	service, err := New(
		opener,
		fakeRefresher{err: conversation.ErrInvalidStateToken},
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNativeFlowReleasesMeaningfulAudioOnlyAfterFinalInputGate(t *testing.T) {
	...
	service, err := New(opener, fakeRefresher{token: "opaque-state"})
	if err != nil {
		t.Fatal(err)
	}
	defer service.Close()
