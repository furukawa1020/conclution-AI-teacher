...
func promptOnRecognitionMiss(input httpapi.VoiceTurnInput) bool {
	// Only the finite capture window opened by a fresh user gesture may offer
	// a deterministic companion topic after a recognition miss. It never
	// guesses at or requests a repeat of unheard content. Automatic foreground
	// listening stays silent on a miss so synthesis cannot feed another VAD
	// cycle.
